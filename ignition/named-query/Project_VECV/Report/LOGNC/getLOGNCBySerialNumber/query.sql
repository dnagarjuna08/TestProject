...
WHERE (TE.[SerialNumber] IN (SELECT VALUE FROM string_split( :SerialNumber ,','))) and [MST_Event_Id]=8 and OPNC.MST_NCStatus_Id!=2
AND TE.CreatedDate BETWEEN :StartDate AND :EndDate
AND ( :workstation = '-1,' or :workstation = '' or TE.MST_WorkStation_Id IN (SELECT VALUE FROM string_split(  :workstation  ,',')) )
AND ( :Area = '-1,' or :Area = '' or TE.MST_Area_Id  IN (SELECT VALUE FROM string_split(  :Area  ,',')) )
AND ( :Line = '-1,' or :Line = '' or TE.MST_Line_Id  IN (SELECT VALUE FROM string_split(  :Line  ,',')) )
AND ( :BOM = '-1,' or :BOM = '' or TE.M_BOM_Id  IN (SELECT VALUE FROM string_split(  :BOM  ,',')) )
