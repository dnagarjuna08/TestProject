...
SET NOCOUNT ON
	...
	left outer join TRN_StationEvents SE WITH(NOLOCK) on Se.SerialNumber=po.serial_number

	where (se.MST_Event_Id <>5 or se.MST_Event_Id is null)
	and PO.MST_Area_Id=@MST_Area_Id order by PO.psn
	
	
	 delete from @PRODUCTION_ORDER 
	 where serial_number =(select top 1 SerialNumber from  TRN_StationEvents where  MST_Event_Id=5 and SerialNumber in(select serial_number from @PRODUCTION_ORDER ))
	...
	set ActionT=0
	 where OrderStatusid <> 4
	END
select top 20 order_number as 'Order Number',serial_number as 'Serial Number' from @PRODUCTION_ORDER  
order by psn
END
