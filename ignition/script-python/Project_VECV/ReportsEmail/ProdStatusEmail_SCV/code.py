def ProdStatus():
	try:
		...
		workstation_order = [
		    (339, "MAIN LINE WASHING MACHINE"),
		    (357, "DRIVE HEAD WASHING MACHINE"),
		    (349, "EV DIESEL QUALITY GATE"),
		    (338, "WHEEL END QUALITY GATE"),
		    (356, "DIFF CASE SA QUALITY GATE"),
		    (364, "MAIN LINE QUALITY GATE"),
		    (369, "PDI"),
		]
		
		# Define flow structure (grouped as columns, with a flow downward)
		flow_structure = [
		    ["DRIVE HEAD WASHING MACHINE", "EV DIESEL QUALITY GATE"],
		    ["WHEEL END QUALITY GATE"],
		    ["DIFF CASE SA QUALITY GATE"],
		    ["MAIN LINE WASHING MACHINE", "MAIN LINE QUALITY GATE", "PDI"],
		]
		
		# Initialize email body
		email_body = (
		    "<p>Dear All,</p>"
		    "<p>Please find below the <b>SCV Production Status Report</b> for date " + today + ": All Shifts</p>"
		        ...
		        if workstation:
		            try:
		                results = system.db.runNamedQuery(
		                    "Project_VECV/Report/ProductionStatus/getData2_SCV",
		...
		try:
		    monthly_results = system.db.runNamedQuery(
		        "Project_VECV/Report/ProductionStatus/getData_SCV",
		        {"WorkStation": 369, "FromDate": from_date_month_str, "ToDate": toDate}
		    )
		    monthly_results2 = system.db.runNamedQuery(
		                                "Project_VECV/Report/ProductionStatus/getData_SCV",
		                                {"WorkStation": 364, "FromDate": from_date_month_str, "ToDate": toDate}
		                            )
		    email_body += generate_table(monthly_results2, "MONTHLY Production (TAQG04) -> "+current_month)
		...
		try:
		    system.net.sendEmail(
		        ...
		        subject="MES ||SCV AXLE SHOP || Production Status Report (" + today + " All Shifts)",
		        body=email_body,
		        html=1,
		        to=email_list
		    )
