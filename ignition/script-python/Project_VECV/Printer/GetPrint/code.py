...
#	else:
#	    shift='C'
	BOMNumb=serialNumber[2:].split('-')[1]
	if "ID" in BOMNumb:
		SerailNumberBigSize=serialNumber[2:].split('-')[1]
	else:
...
def getDHVorPrint(IP,port):
		
		scriptName="Printer"
	#	printerIP = '10.109.12.224'
		printerIP=IP
		...
		^LL0406
		^LS0
		^FT230,150^A0N,90,70^FH\^FD"""+barcodeData+"""^FS
		^PQ1,0,1,Y
		^XZ"""
		NoOfPrint=1
	#	port = int(9100)
		...
		try:
			...
			screenName ='Quality PDI' 
			filePath=str('Peoject Library/Project_VECV/Printer/getDHVorPrint')
			errorFunction = str('Scripting:FunctionName->getDHPrint')
			errorType=str('Application')
			errorCode = str(Project_VECV.Common.GenerateUUID.getUniqueUUID()) 
			description=str(sys.exc_info()[1]) #error description
